...
def replace_lam_alef(unshaped_word):
    ...
    for i in range(len(unshaped_word)):
        if not is_haraka(unshaped_word[i]) and unshaped_word[i] != DEFINED_CHARACTERS_ORIGINAL_LAM:
            letter_before = unshaped_word[i]

        if unshaped_word[i] == DEFINED_CHARACTERS_ORIGINAL_LAM:
...
def get_lam_alef(candidate_alef, candidate_lam, is_end_of_word):
    ...
    if DEFINED_CHARACTERS_ORIGINAL_LAM == candidate_lam:
        if DEFINED_CHARACTERS_ORIGINAL_ALF_UPPER_MDD == candidate_alef:
            reshaped_lam_alef = LAM_ALEF_GLYPHS[0][shift_rate]

        if DEFINED_CHARACTERS_ORIGINAL_ALF_UPPER_HAMAZA == candidate_alef:
            reshaped_lam_alef = LAM_ALEF_GLYPHS[1][shift_rate]

        if DEFINED_CHARACTERS_ORIGINAL_ALF == candidate_alef:
            reshaped_lam_alef = LAM_ALEF_GLYPHS[2][shift_rate]

        if DEFINED_CHARACTERS_ORIGINAL_ALF_LOWER_HAMAZA == candidate_alef:
